...
public class MainViewModel : ViewModelBase
{
    ...
    private void UpdateLive(HwSample sample)
    {
        ...
        {
            basicUpdates.Add(new MetricUpdate("basic-cpu-temp", "CPU °C", "°C", sample.CpuTemp));
        }

        if (sample.CpuMHz.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-cpu-mhz", "CPU MHz", "MHz", sample.CpuMHz));
        }

        if (sample.GpuTemp.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-gpu-temp", "GPU °C", "°C", sample.GpuTemp));
        }

        if (sample.GpuCoreMHz.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-gpu-core", "GPU Core MHz", "MHz", sample.GpuCoreMHz));
        }

        if (sample.GpuMemMHz.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-gpu-mem", "GPU Mem MHz", "MHz", sample.GpuMemMHz));
        }

        if (sample.CpuFanRpm.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-cpu-fan", "CPU Fan", "RPM", sample.CpuFanRpm));
        }

        if (sample.GpuFanRpm.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-gpu-fan", "GPU Fan", "RPM", sample.GpuFanRpm));
        }
        else if (sample.GpuFanPct.HasValue)
        {
            basicUpdates.Add(new MetricUpdate("basic-gpu-fan", "GPU Fan", "%", sample.GpuFanPct));
        }
        ...
        if (sample.CpuFanRpm.HasValue)
        {
            cpuUpdates.Add(new MetricUpdate("cpu-fan", "Вентилятор CPU", "RPM", sample.CpuFanRpm));
        }
        ...
        if (sample.GpuFanPct.HasValue && !sample.GpuFanRpm.HasValue)
        {
